...
//feduss
Blockly.Flyout.prototype.insertBlock = function(xmlList, index){
  /*
   * this.flyout_.show(xmlList) -> Blockly.Flyout.prototype.show = function(xmlList) of flyout.js,
   * until this.layout_(contents, gaps)
   */
  ...
  //comment by feduss, but code is of mit: dividing block by categories, i think
  ...
  /*
   * Test by feduss based on same codes of Blockly.Flyout.prototype.layout_ = function(contents, gaps) in flyout.js
   */

  var originBlock = contents[index].block;
  //var root = block.getSvgRoot();
  //var blockHW = block.getHeightWidth();

  //alert("BlockName: " + block.toString());

  /*
   * this.addBlockListeners_(root, block, rect) -> Blockly.Flyout.prototype.addBlockListeners_ = function(root, block, rect)
   *
   *  but, in particular, i'm using the code of
   *
   * this.blockMouseDown_(block) -> Blockly.Flyout.prototype.blockMouseDown_ = function(block)
   * in this two rows:
   * 1) this.listeners_.push(Blockly.bindEventWithChecks_(root, 'mousedown', null, this.blockMouseDown_(block)))
   * 2) this.listeners_.push(Blockly.bindEventWithChecks_(rect, 'mousedown', null, this.blockMouseDown_(block)));
   *
   * and i'm executing it directly, without a listener
   */

  var block = originBlock;
  var flyout = this;
  flyout.startDragMouseY_ = 10;
  flyout.startDragMouseX_ = 10;
  Blockly.Flyout.startBlock_ = block;
  Blockly.Flyout.startFlyout_ = flyout;

  /*In theory, i have to execute -> Blockly.Flyout.prototype.onMouseUp_, because
   *i have to simulate the mouse up event, and not the Blockly.Flyout.prototype.onMouseMoveBlock_
   *because i set manually the x,y coordinates of the block.
   *
   * In particular, in this function i need only:
   * this.createBlockFunc_(Blockly.Flyout.startBlock_)
   *
   * and, in the last, only this line:
   * var block = flyout.placeNewBlock_(originBlock)
   *
   * The following lines are from the placeNewBlock_ function, without the original comments
   * and the scroll part (because i don't have the flyout)
  */
  ...
  //block2.onMouseDown_(e);
  Blockly.dragMode_ = Blockly.DRAG_FREE;

  //Nota, queste due righe commentate danno problemi quando si inserisco molti blocchi
